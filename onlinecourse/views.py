...
def get_enrolled(user, course):
    enrolled = False
    if user.id is not None:
        # Check if user enrolled
        enroll_set = Enrollment.objects.filter(user=user, course=course)
        if enroll_set.count() > 0:
            enrolled = enroll_set[0]
...
def submit(request, course_id):
    course = get_object_or_404(Course, pk=course_id)
    user = request.user
    enroll = get_enrolled(user, course)
...
def show_exam_result(request, submission_id):
    ...
    course = get_object_or_404(Course, pk=submission.enrollment.course.id)
    grade = 50
